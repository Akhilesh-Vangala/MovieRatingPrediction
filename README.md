...
Statistical hypothesis testing analysis of movie ratings data examining relationships between popularity, release year, viewer demographics, and rating patterns across 400 movies.
...
```bash
git clone https://github.com/Akhilesh-Vangala/MovieRatingAnalysis.git
...
- **Language**: Python
- **Key Libraries**: NumPy, Pandas, SciPy (for statistical tests)
- **Statistical Methods**: Non-parametric hypothesis testing
- **Dataset**: 400 movies rated by 1,097 participants
- **Significance Level**: α = 0.005 (per-test, as per Benjamin et al., 2018)
- **Data Source**: Replication dataset from Wallisch & Whritner (2017) research

## Key Findings

1. **Popularity Effect**: More popular movies receive significantly higher ratings
2. **Temporal Differences**: Newer and older movies show statistically significant rating differences
3. **Gender Patterns**: 12.5% of movies exhibit gender-based rating differences
4. **Family Structure**: Only 1.75% of movies show an only-child effect
5. **Viewing Context**: 1.5% of movies demonstrate social watching effects
6. **Franchise Consistency**: 87.5% of major franchises show inconsistent quality across movies
7. **Decade Differences**: Movies from the 1990s and 2000s are rated significantly differently
...
Data source: Movie ratings replication dataset from Wallisch, P., & Whritner, J. A. (2017). Strikingly Low Agreement in the Appraisal of Motion Pictures. Projections, 11(1), 102-120.
